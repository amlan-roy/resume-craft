...
if ! echo "$description" | grep -q '## Summary'; then
  ...
  exit 1
elif ! echo "$description" | awk '/## Summary/{flag=1;next}/##/{flag=0}flag' | awk 'length($0) < 5 && NR > 1'; then
  echo "Summary section has less than 5 characters"
  exit 1
elif ! echo "$description" | awk '/## Testing done/{flag=1;next}/##/{flag=0}flag' | awk 'length($0) < 5 && NR > 1'; then
