...
const StepTwo: React.FC<StepTwoProps> = ({
  ...
  const generateResumeUsingChatgpt = async () => {
    if (generateButtonState === "in-progress") return;
    setGenerateButtonState("in-progress");
    ...
    const res = await axios.post(
      `${process.env.NEXT_PUBLIC_SITE_URL}/api/ai-resume-gen`,
      {
        baseResumeData: removeSensitiveInformation(JSON.parse(baseResumeData)),
        jobDescription: jdInputValue,
      }
    );

    try {
      const enteredData = res.data.response;
      const enteredDataObjWithSensitiveInfo =
        enteredData &&
        addBackSensitiveInformation(enteredData, JSON.parse(baseResumeData));
  ...
  const [generateButtonState, setGenerateButtonState] = useState<
    "neutral" | "in-progress" | "success" | "fail"
  >("neutral");
  ...
  const getGenerateButtonLabel = (
    ...
    switch (state) {
      case "fail":
        return "Try again...";
      case "in-progress":
        return "Generating resume data using AI...";
      case "success":
        return "Generated resume data. Click here to generate again";
      case "neutral":
        return `Generate resume data using AI`;
      default:
        return `Generate resume data using AI`;
    }
  };
    ...
    <Card className="w-full" {...props}>
      <CardHeader>
        <h2 className="text-xl font-semibold">
          {isExperimentEnabled(EXPERIMENT_NAMES.RESUME_GEN_USING_GPT)
            ? "Step 2: Genetate the resume data using chatGPT"
        ...
        <div className="flex gap-9 p-6  lg:flex-row flex-col-reverse items-center">
          <div className="w-auto">
            <p>
              ...
              and the entered job description after clicking on the below
              buttons
              <br />
              <br />
      ...
      <CardContent>
        {isExperimentEnabled(EXPERIMENT_NAMES.RESUME_GEN_USING_GPT) ? (
          <Button
            type="button"
            onClick={generateResumeUsingChatgpt}
            disabled={generateButtonState === "in-progress"}
          >
            {getGenerateButtonLabel(generateButtonState)}
          </Button>
        ) : (
          <>
            ...
            <div className="mt-12">
              <Label className="text-brand-neutral-11">
                Enter the JSON response generated after entering the last
                command in chatGPT
              </Label>
              <Textarea
                placeholder={
